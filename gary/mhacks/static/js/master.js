...
var lineGraphHelper = function(options, data){  
    ...
    var graph = d3.select("#custom-file").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
...
var chloroplethGraph = function(options){
  ...
  var svg = d3.select("#custom-file").append("svg")
  .attr("width", width)
  .attr("height", height);

  queue()
  .defer(d3.json, options.matrix)
  ...
  console.log(options.matrix, options.filename);
